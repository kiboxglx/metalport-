...
import { Plus, Minus, Trash2, Package, Tent as TentIcon, ShoppingCart, Calendar, User } from 'lucide-react';
import { Product, Tent } from '@/types/database';
...
const NewRental: React.FC = () => {
  ...
  const handleAddTent = () => {
    ...
    if (existingItemIndex >= 0) {
      ...
      toast.success('Tenda adicionada à locação');
    }
  ...
  const handleRegisterRental = async () => {
    ...
    if (productItems.length === 0 && tentItems.length === 0) {
      toast.error('Adicione pelo menos um item (produto ou tenda) à locação');
      return;
    }
    ...
    try {
      ...
      const tentItemsData = tentItems.map(item => ({
        tent_id: item.tentId,
        quantity: item.quantity,
        unit_price: item.dailyRate,
      }));
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <Card>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold text-foreground">Adicionar Tendas</h2>
            </div>

            <div className="space-y-4">
              <div className="space-y-2">
                <label className="block text-sm font-medium text-foreground">Tenda</label>
                <select
                ...
                >
                  ...
                  {tentsList.map(tent => (
                    <option key={tent.id} value={tent.id}>
                      {tent.name} - {tent.size || 'N/A'} (Estoque: {tent.total_stock})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setTentQuantity(Math.max(1, tentQuantity - 1))}
                  ...
                  <input
                    ...
                    min="1"
                    value={tentQuantity}
                    onChange={(e) => setTentQuantity(Math.max(1, parseInt(e.target.value) || 1))}
                  ...
                  />
                  <button
                    onClick={() => setTentQuantity(tentQuantity + 1)}
                    className="p-2 bg-secondary hover:bg-secondary/80 rounded-lg transition-colors"
                  >
        ...
        <div className="space-y-4 md:space-y-6">
          <Card className="h-full flex flex-col">
            ...
            <div className="flex-1 space-y-4">
              {tentItems.length === 0 && productItems.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  {tentItems.length > 0 && (
                    <div className="space-y-2">
                      <h3 className="text-xs font-semibold uppercase text-muted-foreground tracking-wider">Tendas</h3>
                      {tentItems.map((item, index) => (
                        <div key={`tent-${index}`} className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg border border-border/50">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <TentIcon className="w-4 h-4 text-primary/70" />
                              <p className="font-medium text-foreground text-sm truncate">{item.tent.name}</p>
                            </div>
                          ...
                          <div className="flex items-center gap-3 ml-2">
                            ...
                            <button
                              onClick={() => handleRemoveTent(index)}
            ...
            <div className="hidden md:block mt-6 p-4 bg-secondary/20 rounded-lg space-y-2 border border-border/50">
              ...
              <button
                ...
                disabled={isSubmitting || (tentItems.length === 0 && productItems.length === 0)}
