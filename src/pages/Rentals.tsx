...
const Rentals: React.FC = () => {
  ...
  const calculatedTotals = useMemo(() => {
    ...
    const chargedDays = formData.chargedDays !== undefined && formData.chargedDays !== ''
      ? parseInt(formData.chargedDays)
    ...
    const subtotal = dailyRate * chargedDays;
    const total = Math.max(0, subtotal - discount + deliveryFee);

    return { dailyRate, subtotal, total, days, chargedDays, deliveryFee };
  }, [selectedProducts, formData.startDate, formData.endDate, formData.chargedDays, formData.discount, formData.deliveryFee, productsList]);
